...
pub(super) fn run_glob_query(
    ...
    let dir_path = match sandbox.resolve(path) {
        Ok(path) => path,
        Err(message) => return SearchOutcome::error(message),
    };
    let search_pattern = match glob_search_pattern(sandbox, &dir_path, pattern) {
    ...
    if matches_with_metadata.is_empty() {
        ...
        return SearchOutcome::ok(display);
    }
...
pub(super) fn glob_search_pattern(
    ...
    if sandbox.restrict_to_workspace {
        let mut allowed_roots = vec![sandbox.workspace.clone()];
        allowed_roots.extend(sandbox.extra_allowed_dirs.iter().cloned());
