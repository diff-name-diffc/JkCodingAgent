...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-9 w-full items-center justify-between gap-2 rounded-lg border border-border bg-input px-3 text-sm text-foreground transition-colors duration-normal ease-motion",
      "hover:border-border focus:outline-none focus:ring-2 focus:ring-ring/20 focus:border-ring",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      sideOffset={4}
      className={cn(
        "z-[400] max-h-72 min-w-[var(--radix-select-trigger-width)] overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-medium animate-fade-in",
        className,
      )}
      {...props}
    >
