...
| UI-28 | P1 / L | 核心功能映射回归 | 10–23,25 | DOING | zcode（会话领取，交接同事验证） | 隔离环境走查：流程 1 主链路 + 编辑重发 + 流程 2/5/6/7 运行态通过（第二批）；停止分支/删除会话下拉登记自动化限制，见第 22 节 |
...
| UI-33 | P2 / S | 后端 CloakBrowser 用户可见串归位 | 26 | REVIEW | claude（会话领取） | `c1090ed`；工具描述/错误/状态消息/子智能体系统提示词 CloakBrowser→浏览器（sidecar/Node 等技术详情保留）；纯字符串改动，cargo 507 passed 不回退、contract:check 115:112 不变 |
...
| 2026-09-07 | J1：UI-24⑥ 终端高度拖拽隔离（drag 状态从 useProjectPanels 下沉到 ShellTerminalPanel 本地 state，重渲染限面板子树；新增 terminalDragBounds 纯函数与 resolveWorkspaceBudget 垂直钳制同口径消除松手跳变，4 test；键盘通道零改动；dragCleanupRef 卸载兜底防拖拽中途 terminate 悬挂） | `60d0c79` |
| 2026-09-07 | J2：UI-24④ live-state 逐 token setState 改 rAF 合帧 + 删 usage 死字段（删除 current-session 立即 setState，流式渲染由每 token 一次降为每帧至多一次，ChatShell readonly rAF 订阅不再被父层击穿；顺带删除零消费者的 activeUsageStats/usageClockNow/activeUsageStatsReceivedAt 死字段与每秒空转时钟、createEmptyUsageStats；runUsageUpdated 仅保留 refreshSessionTokenUsage） | `a14773e` |
| 2026-09-07 | J3：UI-27 死类/死动画清理（.ai-toolbar、.chat-session-card 家族、.dispatcher-message-list 7 条、.dispatcher-streaming-caret、.file-viewer-code 旧 Shiki 通道、@keyframes toast-in/pulse/rail-active-breathe/message-in/stream-caret、.make-run-btn；styles:report 924→923，0 无引用） | `b81136e` |
...
## 16. M3 余量（UI-24 遗留④⑥ + UI-27 + UI-33）任务记录（第 5 节模板）

任务：UI-24（遗留④⑥ 切片；profile 验收与⑦⑧仍遗留）
负责人：claude（会话领取）
开始/完成日期：2026-09-07
基线/结果 commit 或 PR：基线 `3edcd81`；结果 `a14773e`（④）/`60d0c79`（⑥），两 commit 独立可回退
实现文件与范围：
④ `useLiveSessionState.ts`（删「当前会话立即 setState」路径，流式 token 改由既有 rAF 批量通知统一送达；删 usage 死字段 + 每秒空转时钟）、`dispatcherSessionStore.ts`（类型 + createIdleLiveSessionState 删三死字段）、`useDispatcherActions.ts`/`event-channel.ts`/`useArchitectureChat.ts`（删死字段写入，runUsageUpdated 仅保留 refreshSessionTokenUsage）、`dispatcherChatUtils.ts`（删零引用 createEmptyUsageStats）
⑥ `workspace-budget.ts`（新增 terminalDragBounds 纯函数，resolveWorkspaceBudget 垂直钳制改调它单一出处；+4 test）、`ShellTerminalPanel.tsx`（dragHeight 本地 state + handleResizeStart + dragCleanupRef 卸载兜底，getValue 改读 dragHeight ?? height，把手门控改 onResizeCommit 存在性）、`useProjectPanels.ts`（删 dragTerminalHeight/handleTerminalResizeStart，terminalHeight 收敛为纯 prefs）、`ProjectPage.tsx`（删 onResizeStart 透传）
对应问题：审计 A11（遗留④⑥）、UI-24a 记录「④⑥ 需与 profile 数据协同评估」、设计 §6「流式响应→滚动锚点可恢复」
测试命令及结果：④ 无新增纯函数（改触发时机/删死字段），全量 vitest 420 passed；⑥ terminalDragBounds 4 case；build/lint(--max-warnings 0)/styles:report(0 无引用) 全绿；零 Rust 改动
截图：遗留（性能验收不适用截图）
人工走查：⑥ 终端拖拽流畅性/松手持久化/拖拽中途切项目、④ 流式输出手感（逐 token → 按帧批量，60Hz 不可分辨）——遗留 UI-28/30
风险/回退：两 commit 独立可回退；④ revert 即恢复立即路径（仅性能退化）；⑥ revert 即回到 ProjectPage 全树拖拽重渲染（无功能损失）；⑥ 键盘通道（useSplitterKeyboard 即时提交）零改动，splitter-step 14 case 保护
阻塞或剩余事项：**UI-24 状态保持 DOING**——「附实际 profile」验收、⑦SessionPanel/侧栏会话列表虚拟化、⑧保活监听者累积盘点（归 UI-30）仍遗留；④ 的结构性拆分（布尔 selector + 写入器分离）留待 profile 数据决定
验收人/日期：待人工（UI-28/30/31）

任务：UI-27
负责人：claude（会话领取）
开始/完成日期：2026-09-07
基线/结果 commit 或 PR：基线 `a14773e`；结果 `b81136e`（死类/死动画）/`10f4a8f`（孤儿 rightPanel）/`a74e0b2`（文档），三 commit 独立可回退
实现文件与范围：
死类清理 `App.css`（.chat-session-card 家族、.dispatcher-message-list 7 条、.dispatcher-composer-shell/-button、.dispatcher-streaming-caret、.file-viewer-code 旧 Shiki 通道、.make-run-btn、@keyframes toast-in/pulse/rail-active-breathe/message-in/stream-caret、user-select 清单 .ai-toolbar）、`tailwind.css`（.ai-toolbar 定义）
孤儿 rightPanel `workspace-prefs.ts`（字段+sanitize）、`workspace-budget.ts`（chrome rightPanelMin/Max、input/output 字段、降级分支、五步→三步）、`ProjectPage.tsx`（恒关入参）、`useProjectPanels.ts`（过时注释）、两测试按新口径改写
文档 `AGENTS.md`（主视图删 AnalyticsDashboard/ProjectRail、模块表 project/analytics.rs 与 platform/usage.rs、概述「用量分析」→「用量记录」）、`README.md`/`README_ZH.md`（功能清单「Analytics Dashboard/分析看板」→「Usage Tracking/用量记录」）
对应问题：UI-18 记录「rightPanel 无消费者——UI-27 评估移除」、UI-16 记录「.file-viewer-code 旧 Shiki 通道归 UI-27」、UI-08 记录「旧 nezha.* 保留待清理」、风险登记「用量分析旧入口缺失——核实后更新文档」、审计 A05 设计语言叠加
测试命令及结果：全量 vitest 419 passed（workspace-budget/prefs 测试按新口径改写，删除 3 个 rightPanel 相关 case）；build/lint/styles:report(923 定义，0 无引用)/contract:check(115:112) 全绿
截图：N/A（纯清理）
人工走查：亮暗主题视觉无回归——遗留 UI-29
风险/回退：三 commit 独立可回退；`--info` 无 .dark 覆盖经核实为品牌色跨主题恒定设计（非失配），对比度实测登记 UI-29；tailwind.css 68 处 `!important` 为对抗 App.css 未分层规则的活覆盖（file-explorer-context-menu/file-viewer-tab-menu/ai-project-session-*/ai-sidebar-footer-actions 等），删会改变渲染，本批保留登记；`nezha.chat.*` 两键仍在读（非孤儿），UI-08 记录更正为「仅历史命名」
阻塞或剩余事项：tailwind.css 68 处 `!important` 活覆盖的根治（App.css 分层迁移）未纳入本轮，登记后续
验收人/日期：待人工（UI-29）
...
## 19. M3 遗留代码领取（第三批：UI-17 split 行内 word-level 高亮 + UI-22c rag 失败原因拼 stderr 尾部 + UI-14 子智能体轨迹真实 model 字段）任务记录（第 5 节模板）

> 本节延续第 17/18 节模式，领取 M2/M3 任务记录中已登记的代码级功能遗留（「登记后续/登记不实施/另立任务」），非新发现问题，不启用新编号；原任务总表状态不变（仍 REVIEW，运行态人工验收遗留 UI-28/29），证据列追加本节 commit。M4（UI-28–32）为运行态发布验收（截图矩阵/DevTools profile/人工走查），本会话无运行态桌面环境，按「不虚报」原则不领取；UI-24 遗留⑦与 profile 验收维持既有决策（待 UI-30 运行态数据）。**登记信息修正**：UI-14 子智能体 model 字段在第 18 节曾以「需 schema v3→v4 迁移」评估后延期，本批调查确认基线实际已为 v4（v3→v4 已被 projects.branch 死列删除占用，该次变更未在本清单登记），改以 v4→v5 领取，supersede 该延期；schema.rs 头注释与 AGENTS.md 的「v3 基线」陈旧描述随本批校正。其余既有登记（UI-17 遗留③ GitChanges/GitHistory 缓存同步依赖 ContextNav keep-alive 架构决策、UI-27 `!important` 根治属样式链非功能、图标三方言统一需运行态走查、Git 历史触底自动加载有既有决策）维持不领取。源码基线 `78b9019`。

任务：UI-17（遗留领取：split 并排 diff 行内 word-level 高亮）
负责人：claude（会话领取）
开始/完成日期：2026-09-08
基线/结果 commit 或 PR：基线 `78b9019`；结果 `62ad732`，单 commit 可回退
实现文件与范围：新增 `lib/word-diff.ts`（零依赖 token 级 LCS 纯函数：CJK 感知分词——Han/假名逐字、拉丁按词、空白独立 token，join 恒等原文可精确重建；空白 token 永不高亮降噪〔缩进/连排空白差异不产生噪声〕；char 4000 / DP cells 250k 双阈值超限返回 null 优雅降级；+8 test）、`lib/git-diff-split.ts`（SplitSide 加可选 `segments` 字段；flushRuns 仅真配对 del↔add 侧调用 diffWords，ctx/empty/未配对侧一律不带——配对关系只在 zip 处存在，单一真相源）、`GitDiffViewer.tsx`（抽 SplitHunkRows 子组件 useMemo 按 hunk 缓存 buildSplitRows——word-diff O(m·n) 不随 viewMode 切换等重渲染重算，顺带收敛原每渲染直调；SplitSideView segments 分支渲染 `.git-diff-word-hl--add/del`，无 segments 回退纯文本；unified DiffLineRow 零改动）、`App.css`（`.git-diff-word-hl` 三类：`rgb(var(--success-rgb)/0.3)`/`rgb(var(--danger-rgb)/0.3)` 伴随令牌——浅深双主题自适应、零硬编码色值、无需 html:not(.dark) 覆盖；α=0.3 强于行背景 0.12/0.10 使变更词跳出）
对应问题：第 18 节 UI-17 split diff 记录「**登记不实施**——行内 word-level 高亮（char-level diff）」；设计 §5.4
测试命令及结果：word-diff 8 case（相同/完全不同/单侧空 null/中英混排/空白降噪/char 阈值/cells 阈值/中间词变更）+ git-diff-split 新增 4 case（配对带 segments/未配对 undefined/ctx 无/超长降级）、3 处配对断言 toEqual→toMatchObject（容忍 segments，对齐断言意图不变，其余 8 case 不受影响）；全量 vitest 475→487 passed；build/lint(--max-warnings 0)/styles:report(925:931——.git-diff-* 族非 .ai-*，定义计数不变，0 无引用)/contract:check(115:112) 全绿；cargo 513 passed（零 Rust 改动确认不回退）
截图：遗留（split 行内高亮观感/亮暗双主题/中英混排/长行 pre-wrap 换行下的高亮连续性/极端不等长降级——tauri 运行态，同 UI-29 矩阵）
人工走查：Git 变更/历史打开 diff → 切「并排」→ 配对变更行仅变更词高亮（del 红/add 绿）、公共前后缀不高亮；中文按字粒度高亮；minified 超长行/大 hunk 自动降级纯文本不卡顿；unified 模式与既有 split 行为零变化——遗留 UI-28/29
风险/回退：单 commit 可回退；segments 为可选字段，split 结构渲染不依赖它，回退后 split 照常工作；最坏阈值超限降级纯文本（行级 add/del 背景仍在）；性能每配对行 O(m·n) 由双阈值 + hunk 级 useMemo 双重保护
阻塞或剩余事项：**登记不实施**——split 虚拟化（大 diff 留后续，与第 18 节既有登记合并跟踪）；高亮粒度为 token 级（词/CJK 字），未做 char 级逐字符 diff（token 粒度噪声更低、契合代码审查场景，如需另立决策）
验收人/日期：待人工（UI-28/29）

任务：UI-22（遗留领取：rag 失败原因拼 sidecar stderr 环形尾部）
负责人：claude（会话领取）
开始/完成日期：2026-09-08
基线/结果 commit 或 PR：基线 `62ad732`；结果 `70f0ef1`，单 commit 可回退
实现文件与范围：后端——`rag/failure.rs`（新增 `RagStderrRing`：Arc<Mutex<VecDeque>> Clone 共享供 'static reader/reaper 持有；容量 20 行、单行截断 300 chars、拼接取最近 5 行、尾串总长钳 600 超限截尾加「…」；push 在锁外完成脱敏/trim/截断、临界区仅内存操作〔持锁禁 I/O〕；空行跳过；`append_stderr_tail` 拼接助手——格式 `{base}｜stderr 尾部：{l1 / l2}` 单行化，空快照 no-op 故所有失败点可无条件调用；`STDERR_DRAIN_GRACE` 80ms；+8 test）、`rag/logs.rs`（`redact_log_text` 提为 pub(crate)：ring 入环前复用与 RagLogStore 同一脱敏，ring 自身保证永不持密，凭据不经 rag_status 泄出）、`rag/manager.rs`（接线净 +31 行全在 spawn_and_handshake_impl 作用域：每次 spawn 新建 ring 按代隔离〔restart 不串代，旧 ring 随任务结束自然 drop〕；stderr reader append 后 push；reaper Ok/Err 两分支与「握手前已退出」路径 drain 宽限 80ms 后 failure_message 拼接；握手超时/健康检查失败直接拼不加宽限〔进程多 hung、空 tail 即 no-op〕；L246/L266 record 点零改动——tail 随 `{error:#}` 自动流入）。前端——零改动（deriveRagRuntimeState 对 message 透明只 trim；reason 渲染于内联 span + title tooltip，单行化不撑破状态行，完整文本 tooltip 可见）
对应问题：第 17 节 UI-22c 记录「**不做（登记）**——握手前退出的真因常只在 sidecar stderr，manager 留最近 N 行 stderr 环形片段拼进 message 的增强未纳入……如需另立小任务」；审计 V06（异常含真实原因）
测试命令及结果：failure.rs 新增 8 case（FIFO 容量/空行跳过/单行截断/sk-与 Authorization 脱敏/拼接格式/空 tail no-op/行数与总长钳制/按代隔离+failure_message 集成），现有 4 case 不变；cargo test 513→521 passed；前端零改动——build/lint/styles:report(925:931 不变)/contract:check(115:112——rag_status DTO 字段不变) 全绿
截图：N/A（失败原因内容增强，展示通道为第 17 节既有内联原因行）
人工走查：制造启动失败（如 sidecar 依赖缺失致 Python traceback）→「未运行」下方原因行内联显示 `{失败串}｜stderr 尾部：{最近行}`，title 悬浮可见全文；含凭据的 stderr 行脱敏为 sk-***/Authorization: ***；运行期 kill sidecar → reaper 记录带尾部；启动成功 → 原因清空——遗留 UI-28/29（需 cargo 重建 + 重启 tauri）
风险/回退：单 commit 可回退（失败原因退回无 stderr 尾部，record/clear/get 与前端透出通道不变，前端零改动无连带）；内存有界 20×300 chars；宽限 sleep 仅失败路径 ≤80ms×3，成功路径零影响；极小时序窗口（宽限后 stderr 仍未 drain 完）接受「偶尔少最后一两行」——消息保留「查看 RAG sidecar 日志」兜底指引
阻塞或剩余事项：无新增。第 17 节既有边界登记（RagFailureLog 语义为「最近一次失败」非「当前状态的因」，前端 stopped 门控已对齐）不变
验收人/日期：待人工（UI-28/29）

任务：UI-14（遗留领取：子智能体轨迹真实 model 字段，schema v4→v5）
负责人：claude（会话领取）
开始/完成日期：2026-09-08
基线/结果 commit 或 PR：基线 `70f0ef1`；结果 `5ab0cad`，单 commit 可回退（**回退注意**：已迁移到 v5 的本地开发库需 `scripts/reset-dev-data.sh` 重置或从 `pre-v5-backup` 快照恢复，v4 代码打开 v5 库会走既有版本不兼容 bail）
实现文件与范围：**双通道方案**——Started 事件加 model（实时源）+ sub_agent_run_traces 加可空 model 列（回放权威源：长任务中 Started 作为首个事件必被 G1-20 容量裁剪逐出，届时回放只能靠持久列）。后端：`agent/db/schema.rs`（SCHEMA_VERSION=5 + 头注释/doc「v3 基线」校正 v5 + 链描述补 v3→v4/v4→v5；`migrate_v4_to_v5`——事务外 VACUUM INTO pre-v5 快照失败留痕不阻断〔ADD COLUMN 非破坏性，快照为规范一致性〕→ 事务内 ensure_sub_agent_trace_table_tx 幂等建表 + pragma_table_info 守卫下 ALTER TABLE ADD COLUMN model TEXT + user_version 同事务推进；init() 挂载点 return 从 v4 块移到 v5 块；历史迁移块零改动；+单步迁移 fixture 测试、4 处备份清理 glob 补 pre-v5〔含 no-legacy 测试既有 pre-v4 缺口顺带修正〕）、`agent/sub_agent/db.rs`（基线 DDL 加 model TEXT；DTO 加 `model: Option<String>`；save_run_trace 加参 + INSERT 列 + ON CONFLICT UPDATE；get_run_trace SELECT 加列；4 处测试调用补参 + roundtrip Some/None 断言）、`runtime/build.rs`（`pub fn model()` 透传 provider.model() 最终解析值——继承父级或子智能体独立配置的统一出口）、`runtime/events.rs`+`runtime.rs`（Started 变体加 `model: String`〔enum 仅 Serialize 无反序列化破坏〕；发射处带真实模型；+序列化测试）、`tool.rs`+`manager.rs`（persist_model 随 save_run_trace 落库；写入链路唯一，graph sidecar call_sub_agent 复用同一 tool 天然覆盖）。前端：`types/sub-agent.ts`（SubAgentRunTrace.model?、SubAgentEvent.data.model?）、`subAgentEventStore.ts`（SubAgentSession.model；applySubAgentEvent Started 分支取值/其余分支保留；hydrateSubAgentTrace 加可选列值参，回放后非 null 权威覆盖）、`chat-shell.tsx`（hydrate 传 trace.model）、`SubAgentExecutionView.tsx`（有 model 显示真实值 + title，老轨迹保留「未记录」muted 兜底——不用当前配置冒充运行记录；零新增 CSS）。文档：AGENTS.md「存储 schema 版本策略」v3→v5 校正 + 迁移链补全
对应问题：第 10 节 UI-14 记录「SubAgentSession/轨迹无 model 字段——按规格显示『未记录』，如需真实模型需后端 schema 变更（另立任务）」；第 18 节节前言以「需 schema v3→v4」延期——本批修正为 v4→v5 领取（supersede）；设计 §5.3
测试命令及结果：cargo test 521→523 passed（v4→v5 单步迁移 fixture：user_version 推进/model 列补齐/既有行保留 NULL/pre-v5 快照生成/重开幂等 + Started 事件序列化 data.model 断言）；db.rs roundtrip 补 Some/None 断言；新增 `subAgentEventStore.test.ts` 4 case（Started 归一化/列值覆盖被逐出情形/列值优先于事件值/老轨迹 undefined 兜底）；全量 vitest 487→491 passed；build/lint/styles:report(925:931 不变)/contract:check(115:112——sub_agent_get_run_trace 命令签名不变，DTO 字段级增量不在命令名级检查范围) 全绿
...
## 22. M4 运行态验收启动（UI-28 隔离环境走查：流程 1 主链路 + 验证脚本入库）任务记录与交接

> 本节开启 M4（UI-28–32 发布验收）。此前各批「本会话无运行态桌面环境，按不虚报原则不领取」的前提在本会话解除：computer-use 辅助功能/屏幕录制权限授予且实测可用，UI-01 基线记录的「坐标点击不可用（-25208）」交互走查阻塞**正式解除**（会话行、设置表单、下拉、悬停按钮等全部交互元素可达）。本批完成 UI-28 的验证环境基建与流程 1 主链路验证，其余流程交接同事按同一环境继续。产品代码零改动；新增仅验证工具脚本与本文档。注意：osascript（System Events）与 CLI `screencapture` 在当前宿主环境**无**辅助功能/屏幕录制 TCC 授权，均不可用——窗口定位/截屏证据走 computer-use 通道（`screenshot` 返回光栅图像；CLI 落盘截图待有授权环境补）。

### 验证环境（同事交接手册）

1. **构建**：`cargo build`（src-tauri；纯前端改动可不重建）+ 脚本自动拉起 `pnpm dev`（vite :1420）——debug 二进制运行时加载 `build.devUrl`，无需 `pnpm build`。
2. **一键启动**：`bash scripts/ui-28-run.sh`。串起：种子数据 → mock LLM（:8802）→ vite（:1420）→ .app 包装壳启动应用（`HOME=/tmp/jka-m4` 隔离）。停止：`kill <app pid>` + `lsof -ti :1420,8802 | xargs kill`。
3. **种子数据**（`scripts/ui-28-seed.sh`，可独立执行）：`/tmp/jka-m4` 全新隔离 HOME（dirs::home_dir 读 HOME 环境变量实现隔离；全新目录保证 DB 按当前 v5 基线创建）；`/tmp/jka-m4-projects/proj-a` git 仓库 56 提交（>50 触发历史「加载更多」）+ 工作区未暂存修改/未跟踪文件；`proj-b` 非 git 目录（项目切换/文件树/浏览器目标页 `index.html`）。
4. **mock 模型**（`scripts/ui-28-mock-llm.mjs`，Node 零依赖）：OpenAI 兼容 `/v1/chat/completions` SSE 流式，监听 `127.0.0.1:8802`。行为分支：末条消息 role=tool → 短确认（工具回合收尾）；带 tools 且用户消息含「创建文件」→ 发起 `write_file` 工具调用；含「长回复」→ 80 行慢速流（测停止/滚动锚点）；其余 → 分块流式回复（带 usage chunk 驱动 token 用量展示）。
5. **应用侧配置**：设置 → 模型服务 → 对话模型 → 添加条目（URL `http://127.0.0.1:8802/v1`、API Key 任意非空〔mock 不校验鉴权〕、模型名称任意〔即请求 model 字段，mock 不消费〕）；模型用途页绑定「聊天主模型」「项目主模型」。
6. **bundle 身份要点**：debug 裸二进制无 bundle_id，自动化工具的原始指针通道被安全策略拒绝（悬停显隐按钮如「重新生成」无法点击）；`scripts/ui-28-run.sh` 用最小 Info.plist 包装壳（CFBundleIdentifier=com.jk.jkcodingagent）启动同一二进制即解锁。悬停按钮操作序列：先 mouse_move hover 消息使按钮显现，再点击（否则点击穿透无效果）。

### 已验证（流程 1 主链路，运行态实测）

- 分类 → 新建会话：「工作」分类计数 0→1，会话行即时出现；会话行 keyboard/pointer 均可达（A9 修复实证）。
- 发送 → 流式 → 完成：mock 流式回复完整渲染；助手消息显示「0.2k tokens」用量（usage chunk 通路）；复制/重新生成/赞/踩操作条齐备（悬停显隐）。
- 会话标题自动更新为最新用户消息；侧栏相对时间刷新。
- 错误路径（附带实证 UI-25c）：API Key 未配置时发送 → runError 显式露出「错误：聊天执行失败：错误：聊天 LLM API Key 未配置。…」+「配置模型」深链按钮 → 点击直达设置「模型服务」页且「对话模型」分类已激活（radio=true）。
- 保存管线（流程 11 部分）：模型条目落库 DB 佐证——`model_library_json` 含 url/model/enabled；用途槽位剥离 url/apiKey/model 仅存 libraryId（符合「引用绑定 + 读取回填」设计）；「已保存」头部指示器在编辑后出现。
- 重启恢复：应用重启后会话/消息/用量完整恢复；模型选择器显示已配置模型（「选择模型」替换空态按钮）。

### 发现登记

- **F1（低｜UI-28 走查项）**：设置页完成模型配置并「已保存」后，**已打开**的聊天视图模型选择器仍显示「尚未配置可用模型，前往设置」——但发送实际可用（后端每次运行实时读库）。即前端设置保存后未失效运行中视图的模型查询，重启后恢复正常。修复方向：`aha_save_settings_v2` 成功后 invalidate 模型库/用途相关 query，或设置关闭时失效一次。
- **观察（非缺陷，自动化限制）**：`ApiKeyInput` 经 AX set_value 不触发 React onChange（AX 值写入不派发输入事件；真实键盘输入正常）；该字段为失焦提交语义，自动化填完后需制造 blur 才落库。已用「显示 API Key 明文切换 + set + 点他处 blur」路径走通。
- **观察（待复核）**：从主页空态「尚未配置可用模型，前往设置」按钮进入设置时，AX 树中分类 radio 初值显示全部未选中（后续手动点选正常）；而聊天错误块「配置模型」深链进入时「对话模型」radio=true 预选正确。两入口理论上走同一 `initialProvidersCategory` 机制，前者是否真未预选需运行态复核（可能与 AX 报告口径有关）。**→ 已结案（第二批）**：运行态截图证实两入口「对话模型」均视觉预选，AX 报 radio=false 属报告口径问题，非缺陷。

### 剩余流程清单（交接）

按第 6 节功能映射矩阵，流程 1 剩余：**停止**（真实中断）——UI 内已不可进一步自动化验证（见下方第二批登记），需人工鼠标复核；流程 3–4（工具/子智能体展开、执行图）未开始；流程 8–10（浏览器、Python、架构画布）未开始；流程 11 剩余保存失败分支；流程 12 剩余删除/清空会话的 UI 路径（Radix 下拉自动化不可达）与资源回收 DB 佐证（`sqlite3 /tmp/jka-m4/.jkcodingagent/jkbot.sqlite3`）。完成记录请追加至本节并更新总表状态。

### 第二批走查记录（2026-09-08，zcode 会话）

> 前置：F1 修复 `9e940bd`（纯前端，vite 热加载无需重建二进制）；全新隔离环境（`cargo build` + `ui-28-run.sh` 种子/mock/vite/应用全量重建）。本批无产品代码改动，仅 `scripts/ui-28-mock-llm.mjs` 补请求进入/结束/提前断开日志（判别停止行为用，随批提交）。

**F1 修复运行态复验 ✅**——错误深链进设置 → 补填 API Key（明文切换+真实键盘+失焦提交）→ 头部「已保存」→ DB 佐证 `model_library_json LIKE '%sk-mock-test%'`=1、`chat_agent_chat_model_configs_json`/`project_chat_model_configs_json` 均仅含 libraryId 无凭据（引用绑定设计 ✅）→ 关闭设置 → **已打开聊天视图模型选择器即时由「尚未配置可用模型，前往设置」变「选择模型」（下拉含 mock-chat），未重启**。修复生效。附带结案前批「深链分类预选待复核」观察：AX 树报 radio=false 为报告口径，截图证实「对话模型」视觉已预选，非缺陷。

**流程 1 剩余——编辑重发 ✅**：消息操作条「编辑并重新发送」（悬停显隐，先 mouse_move hover 再点击可达）→ 输入框进入编辑态（「正在编辑消息」+取消按钮+原文回填）→ 发送 → 失败轮次被截断替换为新回复（流式渲染 + 0.2k tokens + 复制/重新生成/赞/踩操作条）。

**流程 1 剩余——停止 ⚠️（自动化限制登记，非缺陷定性）**：发送「长回复」流式中 5 次点击停止（3 次坐标精确命中红色停止方块〔截图佐证〕+ 2 次 AX 元素点击，均在流式窗口内）均未中断——mock 日志佐证请求完整跑 9680ms 无提前断开、DB assistant 2220 字符（80 行全量）+ usage 落库。判别实验：空闲态向输入框键入文本后坐标点击发送按钮同样无效，而输入框聚焦、消息列表/设置弹窗按钮（非 Tooltip 包裹）同样路径均正常——**footer 区 Tooltip 包裹按钮（发送/停止）不响应 window 级合成点击**，真实鼠标行为无法由此通道证伪；停止真实中断（含后端 `dispatcher_stop_run` → `stream_llm_response` 取消 select 链路）需人工鼠标复核后定性。

**流程 6 变更→diff→暂存→提交 ✅**：变更列表（已修改 1：history/log.txt〔M〕；未跟踪 2：.jkcodingagent/、untracked-scratch.txt〔U〕）；0 暂存时提交按钮禁用+「没有已暂存的变更」守卫 ✅；点击文件行 → 详情面板 unified diff（hunk 头、上下文行、新增行绿色、未暂存标记）；行内 + 暂存 → 移入已暂存组（按钮变 −）、提交区文案「提交暂存的 1 个文件」、AI 生成提交信息按钮出现、提交启用；填提交信息提交 → git log `0ecf7a3` + 工作区仅剩两个未跟踪项（**未波及未跟踪区** ✅）→ 列表自动刷新回守卫态。git 命令行双佐证。

**流程 2 项目 A→B→A ✅**：项目列表注册 proj-a（最近项目）；打开 proj-b → 独立会话列表（不串 proj-a 会话）、非 git 项目头部无分支 pill、终端 checkbox=0（与 proj-a 的 =1 互不串台，per-workspace 状态隔离 ✅）；回项目网格 → 最近排序正确（proj-b「刚刚」> proj-a「8 分钟前」）→ 切回 proj-a：变更页签激活、未跟踪列表与 diff 视图恢复、终端保持打开且 PTY 存活。

**流程 5 文件树→文件→修改 ✅**：文件树目录展开/文件选中高亮；notes.md 打开为编辑器 tab，Monaco 行号+Markdown 语法着色，路径工具行（复制完整路径/预览切换）；编辑器输入文本插入成功，900ms debounce 自动保存落盘（磁盘 grep 佐证已写入 notes.md）。tab 脏点存续窗口 900ms 短于自动化往返时延，视觉确认留 UI-29（非缺陷）。

**流程 7 终端输出→隐藏→恢复 ✅**：dock 打开 → PTY 就绪（zsh 提示符 `jk@jmini proj-a $`）；执行命令（自动化键入有竞态散落，但 PTY 往返真实——zsh 对乱串回显 command not found）；点 − 隐藏（保留会话）→ dock 收起、会话区恢复全高、StatusDockBar 终端 pill 高亮（挂载未显示态）→ dock 恢复 → **scrollback 完整保留**（散落字符指纹仍在=PTY 未重建）✅；头部双语义按钮（隐藏终端面板〔保留会话〕/结束终端会话〔终止 shell 进程〕）AX 可见。
