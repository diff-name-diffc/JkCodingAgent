...
#[cfg(test)]
mod tests {
    use super::{is_safe_package_spec, parse_packages};
    use serde_json::json;

    #[test]
    fn package_spec_accepts_common_python_packages() {
        assert!(is_safe_package_spec("pandas"));
        assert!(is_safe_package_spec("matplotlib"));
        assert!(is_safe_package_spec("requests>=2.0"));
        assert!(is_safe_package_spec("uvicorn[standard]"));
    }

    #[test]
    fn package_spec_rejects_shell_syntax() {
        assert!(!is_safe_package_spec("-rrequirements.txt"));
        assert!(!is_safe_package_spec("pandas;rm -rf /"));
        assert!(!is_safe_package_spec("pkg && echo bad"));
        assert!(!is_safe_package_spec("pkg/name"));
    }

    #[test]
    fn parse_packages_requires_non_empty_array() {
        let error = parse_packages(&json!({ "packages": [] })).unwrap_err();
        assert!(error.to_string().contains("不能为空"));
    }

    #[test]
    fn parse_complete_code_strips_markdown_fence() {
        let code = super::parse_complete_code(&json!({
            "code": "```python\nprint('ok')\n```"
        }))
        .unwrap();
    ...
    #[test]
    fn parse_complete_code_rejects_blank_code() {
        let error = super::parse_complete_code(&json!({ "code": "```python\n\n```" })).unwrap_err();
        assert!(error.to_string().contains("不能为空"));
    }
}
