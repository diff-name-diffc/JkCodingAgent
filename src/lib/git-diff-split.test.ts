...
describe("buildSplitRows", () => {
  it("对齐的 del/add 变更组 zip 成同一行（左 del、右 add）", () => {
    ...
    expect(rows[0].left).toEqual({ ln: 1, content: "a=1", type: "del" });
    expect(rows[0].right).toEqual({ ln: 1, content: "a=2", type: "add" });
  });
  ...
  it("ctx 边界分组：ctx→del/add→ctx 顺序与分组正确", () => {
    ...
    expect(rows[1].left).toEqual({ ln: 2, content: "old", type: "del" });
    expect(rows[1].right).toEqual({ ln: 2, content: "new", type: "add" });
