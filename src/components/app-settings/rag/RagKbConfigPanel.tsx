...
function isConfigDirty(a: RagKbConfig, b: RagKbConfig): boolean {
  const { qdrant: qa, embedding: ea, logLevel: la } = a;
  const { qdrant: qb, embedding: eb, logLevel: lb } = b;
  return (
    qa.url !== qb.url ||
    qa.apiKey !== qb.apiKey ||
    qa.collectionPrefix !== qb.collectionPrefix ||
    qa.timeout !== qb.timeout ||
    ea.provider !== eb.provider ||
    ea.baseUrl !== eb.baseUrl ||
    ea.apiKey !== eb.apiKey ||
    ea.model !== eb.model ||
...
export function RagKbConfigPanel({ projectPath: _projectPath }: RagKbConfigPanelProps) {
  ...
  // 测试连接：Qdrant 直接测当前草稿端点；Embedding 暂沿用 sidecar 健康检查。
  const runTest = useCallback(
    async (target: "qdrant" | "embedding") => {
      ...
      try {
        if (target === "qdrant") {
          await invoke("rag_test_qdrant", { config: config.qdrant });
        } else {
          await invoke("rag_health");
        }
        setter({ status: "success", message: "连接正常" });
