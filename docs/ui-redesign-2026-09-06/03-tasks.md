...
更新日期：2026-09-08（M4 运行态验收启动：UI-28 隔离环境走查流程 1 主链路通过，验证脚本入库 `scripts/ui-28-*`，剩余流程交接同事继续；M3 遗留代码领取第五批摘要随本批补登）。关联：[审查](01-audit.md) · [设计规格](02-design.md)。
...
| UI-28 | P1 / L | 核心功能映射回归 | 10–23,25 | DOING | zcode（会话领取，交接同事验证） | 隔离环境走查启动：流程 1 主链路通过 + 发现 F1 登记 + 验证脚本 `scripts/ui-28-*`，见第 22 节 |
...
- **观察（待复核）**：从主页空态「尚未配置可用模型，前往设置」按钮进入设置时，AX 树中分类 radio 初值显示全部未选中（后续手动点选正常）；而聊天错误块「配置模型」深链进入时「对话模型」radio=true 预选正确。两入口理论上走同一 `initialProvidersCategory` 机制，前者是否真未预选需运行态复核（可能与 AX 报告口径有关）。

### 剩余流程清单（交接）

按第 6 节功能映射矩阵，流程 1 剩余：长回复分支的**停止**（真实中断）与**编辑重发**；流程 2–12 未开始：项目 A→B→A 切换隔离、流式中打开工具/子智能体返回、执行图、文件树→编辑→改名/关闭、变更→diff→暂存→提交（proj-a 已备三态与 >50 提交）、终端隐藏恢复、浏览器最小化换会话恢复（proj-b/index.html 为目标页）、Python 执行停止/失败、架构画布主题/收起、设置保存失败重试、编辑重发/清空/删除会话资源回收（DB 层可用 `sqlite3 /tmp/jka-m4/jkbot.sqlite3` 直接佐证）。完成记录请追加至本节并更新总表状态。
