...
//! 真正的动作（校验/落库/广播/收口）由宿主拦截完成。这是旧
//! `OrchestratorAgent::execute_loop_tool_calls` 里按工具名特判的重构形态：
//! 特判逻辑从循环体移出为可注入的处理器，循环只负责「有动作/最终答复则收口、
//! 只有可重试错误则继续」的通用规则。

use serde_json::Value;

/// 协议动作（旧 `run_loop::core::LoopProtocolAction` 的 rig 形态）。
#[derive(Clone, Debug)]
