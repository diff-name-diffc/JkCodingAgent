...
阶段进展：`AppSettingsDialog` 已接入 `ai-settings-shell ai-migrated-settings` scope，设置弹窗主 shell、左侧 tab、右侧 header、关闭按钮、通用设置 footer、加载/缺失/错误/已保存状态完成迁移；旧 CLI 智能体路径与版本配置已随 PI SDK 执行图迁移删除，Aha/RAG/SubAgent 内部业务面板保持独立。验证：`pnpm exec tsc --noEmit`、`pnpm lint`、`pnpm build` 通过。
