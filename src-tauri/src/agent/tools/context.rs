...
pub struct ToolContext {
    ...
    /// 工作区根目录。路径边界输入之一：执行入口（`ToolRuntime::execute_tool`）
    ...
    /// 再传入工具执行」的模式注入（根循环：`ToolRuntime::execute_tool`；
    ...
    pub fn normalize_paths(&mut self) {
        ...
        for dir in dirs {
            match dir.canonicalize() {
                ...
                Err(error) => {
                    eprintln!(
                        "dropping unresolvable extra allowed dir {}: {error}",
                        dir.display()
                    );
                }
            }
...
impl std::fmt::Debug for ToolContext {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ToolContext")
            ...
            .field(
                "ssh_review",
                &self.ssh_review.as_ref().map(|_| "<redacted: contains api key>"),
            )
            ...
            .field(
                "app_handle",
                &self.app_handle.as_ref().map(|_| "<present>"),
            )
            .field(
                "llm_provider",
                &self.llm_provider.as_ref().map(|_| "<redacted: contains api key>"),
            )
...
mod tests {
    ...
    fn context_with_dirs(workspace: std::path::PathBuf, extra: Vec<std::path::PathBuf>) -> ToolContext {
        ToolContext {
    ...
    fn normalize_paths_canonicalizes_workspace_and_drops_unresolvable_dirs() {
        ...
        assert_eq!(context.extra_allowed_dirs, vec![temp.canonicalize().unwrap()]);
    }
