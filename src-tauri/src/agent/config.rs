...
mod tests {
    ...
    fn write_if_missing_creates_new_file() {
        let dir = temp_dir("jkcodingagent-config-write-missing");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("new-file.md");

        write_if_missing(path.clone(), "hello").expect("write succeeds");
    ...
    #[test]
    fn write_if_missing_preserves_existing_file() {
        let dir = temp_dir("jkcodingagent-config-write-existing");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("existing.md");
        fs::write(&path, "original").expect("write original");

        write_if_missing(path.clone(), "new-content").expect("write succeeds");
    ...
    #[test]
    fn sync_bundled_prompt_file_creates_file_when_missing() {
        let dir = temp_dir("jkcodingagent-config-sync-create");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("prompt.md");

        sync_bundled_prompt_file(path.clone(), "new content", &[]).expect("sync succeeds");
    ...
    #[test]
    fn sync_bundled_prompt_file_keeps_file_when_content_matches() {
        let dir = temp_dir("jkcodingagent-config-sync-match");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("prompt.md");
        fs::write(&path, "current content").expect("write current");

        sync_bundled_prompt_file(path.clone(), "current content", &[]).expect("sync succeeds");
    ...
    #[test]
    fn sync_bundled_prompt_file_upgrades_legacy_variant() {
        let dir = temp_dir("jkcodingagent-config-sync-upgrade");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("prompt.md");
        fs::write(&path, "legacy content").expect("write legacy");

        sync_bundled_prompt_file(path.clone(), "new content", &["legacy content"])
    ...
    #[test]
    fn sync_bundled_prompt_file_does_not_overwrite_custom_content() {
        let dir = temp_dir("jkcodingagent-config-sync-custom");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("prompt.md");
        fs::write(&path, "my custom content").expect("write custom");

        sync_bundled_prompt_file(path.clone(), "bundled content", &["old default"])
            .expect("sync succeeds");
    ...
    #[test]
    fn sync_bundled_prompt_file_upgrades_first_legacy_variant_that_matches() {
        let dir = temp_dir("jkcodingagent-config-sync-multi-legacy");
        fs::create_dir_all(&dir).expect("create dir");
        let path = dir.join("prompt.md");
    ...
    #[test]
    fn default_summary_model_is_deepseek_v4_flash() {
        assert_eq!(DEFAULT_SUMMARY_MODEL, "deepseek-v4-flash");
    }
}
